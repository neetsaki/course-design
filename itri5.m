function [L1,L2,L3,L4,L5]= itri5(u1,u2,u3,u4,u5,T_a,T_b,T_c,T_d)
...
[mu5,~]=viscosity(w,T5+273.15,p_sat5*1e-3);
...
omega5=V*rho_5*1e3/n5*cp5*(105-T_c)/H_vap5;
[~,mu_water_5]=viscosity(w,T_pine5+273.15,p_sat(T_m5)*1e-3);
Re_f5=5*omega5/(pi*0.038*mu_water_5);
...
alpha_i5 =alpha_in(rho_5,k5,u5,mu5,cp5*1e3);
...
alpha_o5=alpha_out(Re_f5,rho_water5,rho_vapor5,mu_water_5,H_vap5,k_water5,T_sat,T_pine5);
%给一个建议的初值，待查
%alpha_o1=
%给一个建议的初值，待查
%alpha_o2=
...
K5=(0.038/(alpha_i5*0.032)+Rsi*0.038/0.032+0.003*0.038/(d_m*k_pine)+Rso+alpha_o5^-1)^-1;
T=T_sat;
t_m1=(T_a-83)/(log(T-83)-log(T-T_a));   
t_m2=(T_b-T_a)/(log(T-T_a)-log(T-T_b));
t_m3=(T_c-T_b)/(log(T-T_b)-log(T-T_c));
t_m4=(T_d-T_c)/(log(T-T_c)-log(T-T_d));
t_m5=(105-T_d)/(log(T-T_d)-log(T-105));
%所需换热面积
S1=V*rho_1*1e3*cp1*1e3*(T_a-83)/(K1*t_m1);
S2=V*rho_2*1e3*cp2*1e3*(T_b-T_a)/(K2*t_m2);
S3=V*rho_3*1e3*cp3*1e3*(T_c-T_b)/(K3*t_m3);
S4=V*rho_4*1e3*cp4*1e3*(T_d-T_c)/(K4*t_m4);
S5=V*rho_5*1e3*cp5*1e3*(105-T_d)/(K5*t_m5);
%管长L
%@@@@@@@@@@@@@@这里S 应该指的是管外的面积
L1=S1/(pi*0.032*n1);
L2=S2/(pi*0.032*n2);
